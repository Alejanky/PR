...
namespace pepethegame
{
	public class Snakeplayer
	{
		...
		{//SOYALEJANKY Y ETOY BIEN PENDEJO
			...
			shat = y.Load<Texture2D>("Sprites/shot");
			jump = y.Load<Texture2D>("Sprites/Jump");
			temp = y.Load<Texture2D>("Sprites/bullet");
			currentFrame = 0;
			Rows = 3;
			Columns = 3;
			totalFrames = 9;
		...
		private int Move()
		{
			...
			if (oldState.IsKeyDown(Keys.Up))   // hacer algo con el up stroke 
			{
			...
			if (oldState.IsKeyDown(Keys.Down))   // probablemente no hacer cambiio 
			{
				if (oldState.IsKeyDown(Keys.Left))
				{
					totalFrames = 9;
		...
		public void updatelogic(GameTime gametime, Rectangle X)
		{
			...
			if (permiso== true)
				...
				permiso = this.balas.updatelogical(enemy, 1);
				}
		}
